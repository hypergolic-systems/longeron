// OrbitDriver.TrackRigidbody sits behind an isKinematic guard: for kinematic
// rigidbodies, it never re-sets the orbit's velocity field (`vel`) from
// `vessel.velocityD`. What it *does* do is Swizzle() both `pos` and `vel`
// in-place at the end of each call — and Swizzle is a y↔z swap. So for a
// kinematic vessel in TRACK_Phys mode, `vel` flips components every tick.
// `vessel.srf_velocity = orbit.GetVel() - mainBody.getRFrmVelOrbit(orbit)`
// then oscillates between two values, visible as the navball flickering.
//
// Our fix: for Longeron-managed vessels, temporarily clear isKinematic
// while TrackRigidbody runs. The non-kinematic path updates `vel` from
// `vessel.velocityD` (which we maintain explicitly via the rb.velocity
// write in the scene driver), and the swizzles remain symmetric because
...
namespace Longeron.Patches
{
    ...
    static class OrbitDriverKinematicBypass
    {
        ...
        static void Prefix(OrbitDriver __instance, out bool __state)
        {
            ...
            if (!SceneRegistry.Instance.TryGet(v, out _)) return;
