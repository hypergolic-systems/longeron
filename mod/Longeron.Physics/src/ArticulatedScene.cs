...
namespace Longeron.Physics
{
    public sealed class ArticulatedScene
    {
        ...
        {
            ABA.Solve(body, gravity, scratch);
            // Fixed joints contribute qddot = 0; the loop just no-ops them.
        ...
        public SpatialTransform GetWorldTransform(BodyId b)
        {
            ...
            while (i != -1)
            {
                SpatialTransform xup = body.joint[i].JointTransform(body.q[i]) * body.Xtree[i];
                wToBody = wToBody * xup;
                i = body.parent[i];
            }
            return wToBody;
